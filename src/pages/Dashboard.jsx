...
export function Status({ status }) {
  ...
  return (
    <span className={`inline-block text-xs font-semibold px-2 py-0.5 rounded-full ${colors[status]}`}>
      {status}
    </span>
  );
}

/* ── Universal Modal — renders children inside a dark overlay ── */
    ...
    <AnimatePresence>
      ...
      >
        ...
        >
          ...
          {/* Close button sits outside the form scroll area */}
          <button
            onClick={onClose}
...
export function Dashboard() {
  ...
  /* Called by form components after a successful API submission */
  const handleFormSubmit = (data, type) => {
    const newRequest = {
      id: Date.now(),
      status: "Pending",
      skill: data?.requestedStaff?.[0]?.role || data?.employees?.[0]?.name || "General",
      qty: data?.requestedStaff?.[0]?.quantity || data?.employees?.[0]?.quantity || 1,
      startDate: new Date().toISOString().slice(0, 10),
      endDate: "—",
      display:
        type === "organization" ? data?.companyDetails?.companyName
        : type === "private" ? data?.personalDetails?.surname
        : data?.surname || "—",
    };
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="relative w-full">
        ...
        <div className="relative z-10 px-4 pt-5 pb-6 sm:px-6 text-white">
          <div className="flex items-center justify-between mb-6 pt-20">
            ...
            <motion.button whileHover={{ scale: 1.05 }} className="px-3 py-1.5 text-xs sm:text-sm rounded-lg bg-white/20 border border-white/30 backdrop-blur">
              Edit Profile
            </motion.button>
          </div>
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between gap-4">
            <div className="flex items-center gap-3">
              <motion.img whileHover={{ scale: 1.08 }} src="https://via.placeholder.com/60" alt="avatar"
            ...
            <div className="flex flex-col gap-3 sm:items-end">
              ...
              <motion.button onClick={handlePrimaryAction} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                className={`px-5 py-2 rounded-lg text-sm font-semibold text-white shadow-lg ${mode === "Staff" ? "bg-gradient-to-r from-sky-300 to-sky-600" : "bg-gradient-to-r from-sky-600 to-sky-300"}`}>
          ...
          <div className="mt-5 flex gap-3 overflow-x-auto">
            ...
            {mode === "Staff" && <Stat label="Matched Jobs" value={matchedJobs.length} />}
          </div>
        </div>
      </motion.div>
      ...
      <div className="p-4">
        <div className="flex gap-2 mb-4">
          {["Active", "Pending", "Completed"].map((tab) => (
            <motion.button key={tab} whileHover={{ scale: 1.05 }} onClick={() => setActiveTab(tab)}
              className={`px-4 py-2 rounded-full text-sm font-medium ${activeTab === tab ? "bg-sky-500 text-white" : "bg-white text-gray-700"}`}>
              {tab}
            </motion.button>
          ))}
        </div>
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((r) => (
              <motion.div key={r.id} whileHover={{ scale: 1.02 }}
                className="p-4 border rounded-xl flex flex-col md:flex-row md:justify-between gap-4 mb-3">
                <div className="grid grid-cols-2 md:grid-cols-6 gap-3 w-full">
                  {(mode === "Organization" || mode === "Private") && <Field label="Name" value={r.display} />}
        ...
        {mode === "Staff" && matchedJobs.length > 0 && (
          <div className="bg-white p-4 rounded-xl shadow mt-6">
            ...
            {matchedJobs.map((job) => (
              <div key={job.id} className="p-3 border rounded mb-2">{job.skill} - {job.qty} needed</div>
            ))}
          </div>
        )}
      </div>
      ...
      <UniversalModal open={!!modalType} onClose={() => setModalType(null)}>
        {modalType === "organization" && (
          <ClientForm1 onSubmit={(data) => handleFormSubmit(data, "organization")} />
        )}
        {modalType === "private" && (
          <PrivateForm onSubmit={(data) => handleFormSubmit(data, "private")} />
        )}
        {modalType === "staff" && (
          <StaffForm onSubmit={(data) => handleFormSubmit(data, "staff")} />
        )}
      </UniversalModal>
